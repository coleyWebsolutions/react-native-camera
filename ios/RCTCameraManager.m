...
- (void)saveImage:(NSData*)imageData target:(NSInteger)target metadata:(NSDictionary *)metadata resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    ...
    if (target == RCTCameraCaptureTargetMemory) {
        [self.bridge.eventDispatcher sendAppEventWithName:@"ImageCaptured" body:@{@"data":[imageData base64EncodedStringWithOptions:0]}];
        return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.bridge.eventDispatcher sendAppEventWithName:@"ImageCaptured" body:@{@"path":responseString}];
    });
}
